    ...
    def get_pretty_results(self, uom, benchmark_list):
        ...
        if 'time' in uom and benchmark_list:
            for gender in benchmark_list:
                gender.min_score = "{}:{:.0f}".format(int(gender.min_score), gender.min_score % 1 * 100) 
                gender.avg_score = "{}:{:.0f}".format(int(gender.avg_score), gender.avg_score % 1 * 100)
                gender.max_score = "{}:{:.0f}".format(int(gender.max_score), gender.max_score % 1 * 100)
