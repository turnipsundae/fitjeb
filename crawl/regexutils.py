...
# Returns score from a comment
# RESULTS_RE = re.compile(r'(\d+:?\d{0,2})\s?(rx|min|rep|cal|point|pt|round|rd)(?im)')
...
# hard to distinguish from bodyweight of individual, put on hold
# WT_SCORE_RE = re.compile(r'(\d+)\s?rx|kg|#|lb|kilo|pound(?im)')
...
def get_score_re(uom):
    ...
    return None, None
    # WT_KWS = ['load', 'loads', 'weight', 'weights',
    #     'total', 'totals']
    # # for unit in WT_KWS:
    #     if unit in uom.lower();
    #         return "WT_SCORE_RE"
